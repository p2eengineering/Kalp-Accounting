...
func IsKwalaAdminAddress(ctx kalpsdk.TransactionContextInterface, userID string) (bool, error) {
	...
	data, err := ctx.GetState(key)
	if err != nil {
		wrappedErr := ginierr.NewInternalError(err, fmt.Sprintf("Failed to fetch kwala admin role for userID: %s", userID), http.StatusInternalServerError)
		logger.Log.Errorf(wrappedErr.FullError())
		return false, wrappedErr
	}
	...
	if data == nil {
		// No data found for the given key
		logger.Log.Infof("No kwala admin role exists for userID: %s", userID)
		return false, nil
	}
	...
	// Check if the user has the Kwala Admin role
	if userRole.Id == userID && userRole.Role == constants.KwalaAdminRole {
		return true, nil
	}
	return false, nil
}

func DenyKwalaAddress(ctx kalpsdk.TransactionContextInterface, kwalaAddress string) error {
	kwalaAddressDenyKey, err := ctx.CreateCompositeKey(constants.KwalaDenyListKey, []string{kwalaAddress})
	if err != nil {
		return fmt.Errorf("failed to create composite key for kwala deny list: %v", err)
	}
	if err := ctx.PutStateWithoutKYC(kwalaAddressDenyKey, []byte("true")); err != nil {
		return fmt.Errorf("failed to put data in kwala deny list: %v", err)
	}
	if err := events.EmitDenied(ctx, kwalaAddress); err != nil {
		return err
	}
	return nil
}

func AllowKwalaAddress(ctx kalpsdk.TransactionContextInterface, kwalaAddress string) error {
	kwalaAddressDenyKey, err := ctx.CreateCompositeKey(constants.KwalaDenyListKey, []string{kwalaAddress})
	if err != nil {
		return fmt.Errorf("failed to create composite key for kwala deny list: %v", err)
	}
	if err := ctx.PutStateWithoutKYC(kwalaAddressDenyKey, []byte("false")); err != nil {
		return fmt.Errorf("failed to put data in kwala deny list: %v", err)
	}
	if err := events.EmitAllowed(ctx, kwalaAddress); err != nil {
		return err
	}
	return nil
}

func IsKwalaDenied(ctx kalpsdk.TransactionContextInterface, kwalaAddress string) (bool, error) {
	kwalaAddressDenyKey, err := ctx.CreateCompositeKey(constants.KwalaDenyListKey, []string{kwalaAddress})
	if err != nil {
		return false, fmt.Errorf("failed to create composite key for kwala deny list: %v", err)
	}
	if bytes, err := ctx.GetState(kwalaAddressDenyKey); err != nil {
		return false, fmt.Errorf("failed to get data from kwala deny list: %v", err)
	} else if bytes == nil {
		return false, nil
