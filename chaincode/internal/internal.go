...
func GetTotalUTXO(ctx kalpsdk.TransactionContextInterface, account string, pageSize int) (string, error) {
	...
	for {
		...
		queryString := fmt.Sprintf(`{
			...
			"limit": %d,
			"skip": %d
		}`, account, constants.UTXO, pageSize, skip)

		logger.Log.Infof("Executing query: %s\n", queryString)
